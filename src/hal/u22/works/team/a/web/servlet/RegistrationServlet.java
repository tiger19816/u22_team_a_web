...
public class RegistrationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //ユーザー情報の有無を取得SQL
        String user_retrieval_sql = "select count(*) from members where mail_address = '" + userMail
                + "' AND password = '" + userPassword + "';";

        //ユーザークレジットカード登録SQL
        String user_credit_card_sql = "INSERT INTO cards (no, number, security_code, expiration_date) ";
        user_credit_card_sql += "VALUES (NULL, '" + userCredit + "', '" + securityCode + "', '"
                + userCreditExpirationDate + "');";

        //ユーザークレジットカードno取得SQL
        String user_credit_no_sql = "select no from cards where number = '" + userCredit + "' and security_code = "
                + securityCode + "";
        ...
        try {
            ...
            if (count == 0) { //ユーザー登録が新規の場合
                ...
                user_information_sql += "null,'" + userMail + "','" + userName + "'," + userSex + ",'" + userAddress
                        + "',";
                user_information_sql += "'" + userBirthdate + "','" + userPassword + "','" + userPhone + "',"
                        + userCreditNo + ");";
